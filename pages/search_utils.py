...
def ai_select_best_with_gpt(keyword: str, df: pd.DataFrame):
    ...
    prompt_lines = [
        ...
        "**请严格遵守以下规则：**",
        "1. 异径直接 → 优先选择异径套产品",
        "2. PVC排水管 → 优先选择扩直口管，避免直通或管件",
        "3. PVC给水管 → 优先选择印尼(日标)PVC-U给水扩直口管",
        "4. 规格匹配 → 精确匹配优先（如dn50匹配dn50，避免dn50*25）",
        "5. PPR三通选择规则：",
        "   - 如果搜索结果中有'等径三通'，必须选择等径三通",
        "   - 如果没有等径三通，选择其他PPR三通产品",
        "   - 优先选择联塑品牌",
        "6. 你的回答必须只包含你选择的产品的**索引数字**，不要有任何其他文字、解释或标点符号。",
        "---",
        f"用户的搜索请求是: \"{keyword}\"",
